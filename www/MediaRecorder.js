    ...
    if (this.state === 'inactive') {
        ...
        var success = function (info) {
            ...
            resolveLocalFileSystemURL(that.src, function (entry) {
                ...
                //addedby Hemant F to fix the cors issue
                nativePath= window.Ionic && window.Ionic.normalizeURL && window.Ionic.normalizeURL(nativePath) || nativePath;
                that.src = nativePath;
                that.requestData();
                that.state = 'inactive';
                that.onstop();
...
MediaRecorder.prototype.requestData = function () {
    if (this.state === 'inactive') {
        ...
        fetch(this.src)
            ...
            .then(function (blob) {
                //added by Hemant F to fix the cors issue
                blob.data = (blob.data) ? blob.data : blob;
